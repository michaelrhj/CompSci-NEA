...
def updateProjectile(initVelocity,xProjectile,yProjectile,angle): #takes in required values as parameters
	...
	if initVelocity>0:
		...
		xDisplacement = initVelocity*math.cos(radians(angle))*time_change 
		yDisplacement = initVelocity*math.sin(radians(angle))*time_change + (0.5*gravity*time_change*time_change)
